...
onto this skill. Do it only when the user asks, one step at a time, telling
the user what each step changes. Never delete or rewrite the user's files
without their yes.

## 1. Retire the old mechanism

This skill carries its own rules and upkeep script. With the user, remove or
disable the old hooks, scripts and any threads section in their instruction
files, so that two mechanisms do not act on the same folder. A section this
skill wrote between `<!-- threads:begin -->` and `<!-- threads:end -->` is
not old: leave it (rerunning `snippet` updates it). Hooks that call this
skill's script are fine too ([porting](porting.md)).

## 2. Put the scope where the script looks
...
`start` and `THREADS.md` list every file the contract cannot read as an
anomaly. Go through them with the user; common cases from older setups:
...
Run `start`: the briefing should list the threads with no anomalies.
`THREADS.md` and the archive indexes are rebuilt by the script; if the old
ones held text written by hand, tell the user it will be replaced.
